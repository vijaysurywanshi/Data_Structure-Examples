/**Toeplitz matrix
SchoolAccuracy: 37.19%Submissions: 8K+Points: 0
...
class Solution {
    isToepliz(A, N, M) {
      ...
      for (let i = 0; i < N - 1; i++) {
              for (let j = 0; j < M - 1; j++) {
                  // Check if the current element is not equal to the one diagonally below and to the right
                  if (A[i][j] !== A[i + 1][j + 1]) {
                      return 0; // If elements in any diagonal are not equal, it's not a Toeplitz matrix
                  }
              }
          }
          return 1;
